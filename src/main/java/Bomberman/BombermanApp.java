...
public class BombermanApp extends GameApplication {
    ...
    protected void initInput() {
        ...
        getInput().addAction(new UserAction("Arrow up") {
            @Override
            protected void onAction() {
                getPlayerComponent().up();
            }
        ...
        getInput().addAction(new UserAction("Arrow down") {
            @Override
            protected void onAction() {
                getPlayerComponent().down();
            }
        ...
        getInput().addAction(new UserAction("Arrow Left") {
            @Override
            protected void onAction() {
                getPlayerComponent().left();
            }
        ...
        getInput().addAction(new UserAction("Arrow right") {
            @Override
            protected void onAction() {
                getPlayerComponent().right();
            }
    ...
    protected void initPhysics() {
        ...
        onCollisionBegin(PLAYER, DOOR, (player, door) -> {
            if (isLoading == false
                    && getGameWorld().getGroup(BAlLOM, ONIL).getSize() == 0) {
                isLoading = true;
        ...
        onCollisionBegin(PLAYER, BAlLOM, (player, enemy) -> {
            if (enemy.getComponent(BALLOM.class).getState() != DIE
                    && getPlayerComponent().getState() != DIE) {
                onPlayerDied();
            }
        });
        onCollisionBegin(PLAYER, ONIL, (player, enemy) -> {
            if (enemy.getComponent(ONIL.class).getState() != DIE
                    && getPlayerComponent().getState() != DIE) {
                onPlayerDied();
            }
        });
